...
class Region(Container, Sized, Iterable, Hashable):
    ...
    def perimeter_in_map(self, map: "Map") -> int:
        total = 0
        for p in self.positions:
            for n in map.neighbors(p, restrict=False):
                if n not in self:  
                    total += 1
    ...
    def price_in_map(self, map: "Map") -> int:
        return self.area * self.perimeter_in_map(map)
...
class Map(UserDict):
    ...
    def neighbors(self, location: tuple[int, int], restrict = True) -> Generator[tuple]:
        l, c = location
        results: set[tuple[int, int]] = set()
        for pos in ((l + 1, c), (l - 1, c), (l, c + 1), (l, c - 1)):
            if (not restrict) or self.inbounds(pos): yield pos
    ...
    def init_regions(self, data: dict[tuple[int, int], str]) -> dict[tuple[int, int], Region]:
        ...
        while to_consider:
            #if not len(to_consider) % 100: print(len(to_consider))
...
if __name__ == "__main__":
    ...
    print(myMap)
    print("\n".join(f"{r} Perimeter:{r.perimeter_in_map(myMap)} Price:{r.price_in_map(myMap)}" for r in myMap.regions))
